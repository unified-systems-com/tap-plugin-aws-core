...
- **`configuration` JSONField** — holds the complete resource configuration as received from AWS, so the graph can carry full metadata without a schema change for every new attribute AWS adds. Its purpose is to capture the raw boto3 responses that will be submitted as audit evidence. **Ruling (George, 2026-09-23): the collector does not persist it for now.** Masking or redacting its contents would destroy it as evidence, so collection is disabled instead: `PERSIST_RAW_CONFIGURATION` in the boto3 collector (`spec-aws-core-collector-v0.md` `req-aws-collector-field-projection-7`) is off, and collected nodes carry `{}`. The field stays on every model with no migration; each collection manifest entry declares where its raw response may carry sensitive values (`req-aws-collector-manifest-6`), which is the work list for turning persistence back on.
...
| ACID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-aws-core-fields-1 | Hybrid Field Approach | Implemented | Key typed fields for queries plus a configuration JSONField for full metadata. The field exists on every model; the collector leaves it `{}` while raw-response persistence is off. | Ruling 2026-09-23: no masking; persistence off via `PERSIST_RAW_CONFIGURATION` (`req-aws-collector-field-projection-7`); sensitive locations tracked in the manifest (`req-aws-collector-manifest-6`). |
