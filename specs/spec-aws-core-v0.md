...
- **Security facts kept where configuration is off** — for the types whose configuration is not stored, three security facts are typed fields instead (ruling 2026-09-23 Q44a; `spec-aws-core-collector-v0.md` `req-aws-collector-field-projection-8`): Lambda `vpc_subnet_ids` and `vpc_security_group_ids` (both empty means not in a VPC), CloudFront distribution `origin_access` (each origin's `oac`, `oai` or `none`), and API Gateway HTTP API `route_authorization_types` (each route's `NONE`, `AWS_IAM`, `JWT` or `CUSTOM`, the last being a Lambda authorizer).
